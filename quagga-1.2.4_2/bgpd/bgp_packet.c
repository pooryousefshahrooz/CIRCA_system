        ...
        if (strcmp(as_path_str,"")!=0)
        {
          ...
          if (time_stamp_ds_for_sender_of_update != NULL)
            ...
            else
            {
              ...
              /* this error probably is because of system initialization which after each link up we set all of the data structures to null */ 
            }
           ...
           if (ret <0)
           {
               zlog_debug("We did not get anything for the received event id and time stamp for prefix are %s %s %s \n",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),root_cause_event_id,caused_time_stamp);
                ...
                /* this is for the case we receive a new GRC message and we are advertising some prefixes received from 
                other peers and they have been fizzled and converged in other root cause events.*/

                strncpy(caused_time_stamp ,"GRC", TIME_STAMP_LENGTH);

                sequence_number_for_event_ids = sequence_number_for_event_ids +1;
                zlog_debug("3. +++++++++++++++++++");
                char char_sequence_number_for_event_ids2[EVENT_ID_LENGTH];
                zlog_debug("4. +++++++++++++++++++");
                char str_router_id2[20];
                char * event_id_sent_by_ground22[EVENT_ID_LENGTH];
                zlog_debug("5. +++++++++++++++++++");
                sprintf(str_router_id2, "%u", peer->local_as);
                zlog_debug("6. +++++++++++++++++++");
                sprintf(event_id_sent_by_ground22, "%u",sequence_number_for_event_ids);
                strcat(event_id_sent_by_ground22, ",");
                zlog_debug("7. +++++++++++++++++++");
                strcat(event_id_sent_by_ground22, str_router_id2);
                zlog_debug("8. +++++++++++++++++++");
                strncpy(root_cause_event_id ,event_id_sent_by_ground22, EVENT_ID_LENGTH);
