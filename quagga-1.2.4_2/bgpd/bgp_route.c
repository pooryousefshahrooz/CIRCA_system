...
bgp_process_main (struct work_queue *wq, void *data)
{
  ...
  {
  zlog_debug ("2************************************* we checked prefix %s for all neighbors and this has fizzled!!!!",inet_ntop(p2->family, &p2->u.prefix, buf2, SU_ADDRSTRLEN));
  message_fizzling_check(inet_ntop(p2->family, &p2->u.prefix, buf2, SU_ADDRSTRLEN));
